...
func BenchmarkCompareWith7z(b *testing.B) {
	...
	// --- 1. ZIP (Deflate) ---
	b.Run("Defl_Pk_Zpr", func(b *testing.B) {
		b.SetBytes(largeSize)
		for i := 0; i < b.N; i++ {
	...
	b.Run("Defl_Pk_7z", func(b *testing.B) {
		b.SetBytes(largeSize)
		for i := 0; i < b.N; i++ {
	...
	b.Run("Defl_Un_Zpr", func(b *testing.B) {
		b.SetBytes(largeSize)
		for i := 0; i < b.N; i++ {
	...
	b.Run("Defl_Un_7z", func(b *testing.B) {
		b.SetBytes(largeSize)
		for i := 0; i < b.N; i++ {
	...
	// --- 2. TAR.ZST (Zipper) ---
	b.Run("Zstd_Pk_Zpr", func(b *testing.B) {
		b.SetBytes(largeSize)
		for i := 0; i < b.N; i++ {
	...
	b.Run("Zstd_Un_Zpr", func(b *testing.B) {
		b.SetBytes(largeSize)
		for i := 0; i < b.N; i++ {
	...
	// --- 3. 7Z (LZMA2) ---
	b.Run("7Z_Pk_7z", func(b *testing.B) {
		b.SetBytes(largeSize)
		for i := 0; i < b.N; i++ {
	...
	b.Run("7Z_Un_Zpr", func(b *testing.B) {
		b.SetBytes(largeSize)
		for i := 0; i < b.N; i++ {
			out := filepath.Join(tmp, "out_7z_zipper")
			os.RemoveAll(out)
			runZipper([]string{"zipper", "x", "-C", out, sevenZipArc})
		}
	})

	b.Run("7Z_Un_7z", func(b *testing.B) {
		b.SetBytes(largeSize)
		for i := 0; i < b.N; i++ {
	...
	// --- 4. Мелкие файлы (ZIP) ---
	b.Run("Sm_Pk_Zpr", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
	...
	b.Run("Sm_Pk_7z", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
